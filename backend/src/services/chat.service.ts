...
export const createChatService= async(
    ...
    const participantIdString= populateChat?.participants?.map((p) => p._id?.toString())

    emitNewChatToParticipants(participantIdString, populateChat)
...
export const validateChatParticipant= async(chatId: string, userId: string) => {
    ...
    if(!chat)
        throw new BadRequestException("User not a participant in chat")
